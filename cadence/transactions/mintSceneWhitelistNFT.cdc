...
transaction(cid: String, description: String) {
    ...
    execute {
        ...
        let price = Arlequin.getArleeSceneWhitelistMintPrice()

        // prepare for payment
        let paymentVault <- self.payerVaultRef.withdraw(amount: price )
        let buyerAddr = self.payerVaultRef.owner!.address

        Arlequin.mintSceneWhitelistNFT(buyer: buyerAddr, cid: cid, description: description, paymentVault: <- paymentVault)
